...
class ModelsEmailUnico{
    ...
    public function valEmailUnico($email, $editarUnico = null, $id = null)
    {
        ...
        if(!empty($this->editarUnico) AND ($this->editarUnico == true)){
            $valEmailUnico->exeReadEspecifico("SELECT idCliente FROM cliente WHERE emailCliente =:email AND idCliente <>:id LIMIT :limit", "email={$this->email}&limit=1&id={$this->id}");
        }else{
            $valEmailUnico->exeReadEspecifico("SELECT idCliente FROM cliente WHERE emailCliente =:email LIMIT :limit", "email={$this->email}&limit=1");
        }        
