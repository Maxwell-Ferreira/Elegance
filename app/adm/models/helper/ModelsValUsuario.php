...
class ModelsValUsuario{
    ...
    public function valUsuario($usuario, $editarUnico = null, $id = null)
    {
        ...
        if(!empty($this->editarUnico) AND ($this->editarUnico == true)){
            $valUsuario->exeReadEspecifico("SELECT idAdm FROM useradm WHERE loginAdm =:usuario AND idAdm <>:id LIMIT :limit", "usuario={$this->usuario}&limit=1&id={$this->id}");
        }else{
            $valUsuario->exeReadEspecifico("SELECT idAdm FROM useradm WHERE loginAdm =:usuario LIMIT :limit", "usuario={$this->usuario}&limit=1");
        }        
